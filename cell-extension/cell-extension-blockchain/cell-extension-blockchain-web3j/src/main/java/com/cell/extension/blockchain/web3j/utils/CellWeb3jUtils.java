...
public class CellWeb3jUtils {
    ...
    public static String transferERC20Token(Web3j web3j,
                                            ...
                                            int decimal,
                                            BigInteger gasLimit,
                                            String contractAddress) throws ExecutionException, InterruptedException, IOException {
        //加载转账所需的凭证，用私钥
        ...
        if (ethGetTransactionCount == null) {
            throw new RuntimeException("ethGetTransactionCount failed");
        }
        //获取nonce，交易笔数
        ...
        if (ethGasPrice == null) {
            throw new RuntimeException("ethGasPrice failed");
        }
        gasPrice = ethGasPrice.getGasPrice();
        //ERC20代币合约方法
        BigInteger transferValue = value.multiply(new BigDecimal("10").pow(decimal)).toBigInteger();
        Function function = new Function(
                "transfer",
                Arrays.asList(new Address(to), new Uint256(transferValue)),
    ...
    public static BigInteger getGasPrice() {
        Box<BigInteger> ret = new Box<>();
        CellWeb3jUtils.doWithWeb3J((v) -> {
            Web3j web3j = v.getWeb3j();
            EthGasPrice price = web3j.ethGasPrice().send();
            ret.setData(price.getGasPrice());
            return ErrorEnums.NO_ERROR;
        });
        return ret.getData();
    }
    ...
    public static String transferERC20Token(
        ...
        CellWeb3jUtils.doWithWeb3J((v) -> {
            ...
            gasPrice = ethGasPrice.getGasPrice();
            //ERC20代币合约方法
            BigInteger transferValue = transferAmount.multiply(new BigDecimal("10").pow(decimal)).toBigInteger();
            Function function = new Function(
                    "transfer",
                    Arrays.asList(new Address(to), new Uint256(transferValue)),
                    Collections.singletonList(new TypeReference<Type>() {
